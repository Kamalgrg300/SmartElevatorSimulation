...
// Java class which acts as a control system for the elevator system. The elevators, floors and passengers are controlled by this class.
...
public class World extends JPanel
{
    ...
    public World(int refreshRate)
    {
        ...
        elevator1 = new Elevator(100, 500);
        elevator2 = new Elevator(250, 500);
        elevator3 = new Elevator(400, 500);
        elevator4 = new Elevator(550, 500);

        floors = Collections.synchronizedList(new ArrayList());
        
        // create 6 floors i.e. 0,1,2, ... ,5
        for (int i = 0; i < 6; i++)
        {
            floors.add(new Floor(100, 200, 250, 350, 400, 450, 500, 550, 100 * (6 - i), 1050, i));
        }
        ...
        timer = new Timer(refreshRate, (e) -> {
            ...
            // increment after every second
            if (counter / 1000 == 1)
            {
                counter = 0;
                ++timeElapsedInSecs;
                
                // after every 5 seconds a new passenger arrives
    ...
    public void spawn()
    {
        ...
        {
            int srcFloor = random.nextInt(6);
            int destFloor;

            do
            {
                destFloor = random.nextInt(6);
            }
            while (destFloor == srcFloor);

            int weight = random.nextInt(120) + 60;

            floors.get(srcFloor).addPassenger(new Passenger(0, 100 * (6 - srcFloor) - 50, weight, srcFloor, destFloor));
        }
    }
